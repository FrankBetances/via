...
/*  crea el `Professional` y abre sesión (`loginSuccess`), navegando a        */
/*  `Pacientes`.                                                              */
/* -------------------------------------------------------------------------- */
...
const ROLES: { value: ProfessionalRole; label: string }[] = [
  ...
  { value: 'psicopedagogo', label: 'Pediatra' },
];
...
export default function RegistroProfesionalScreen({ navigation }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      professional.email = `${colegiado.trim().toLowerCase().replace(/\s+/g, '')}@viaplus.local`;
      professional.passwordHash = '';
      professional.centerId = null;

      const saved = await ProfessionalRepository.createProfessional(professional);
      ...
      showSuccessToast('Registro completado', `Bienvenido/a, ${saved.fullName}.`);
